...
def main():
  ...
  score += 4 - testInput(dirPath + "disconnected.txt", 0)
  print("Score so far: " + str(score))

  score += 5 - testInput(dirPath + "noEuler.txt", 0)
  print("Score so far: " + str(score))

  print("\n-------  From now on all test graphs have Euler circuits  -------\n")

  score += testInput(dirPath + "small.txt", 6)
  print("Score so far: " + str(score))
  
  score += testInput(dirPath + "medium.txt", 5)
  print("Score so far: " + str(score))
 
  score += testInput(dirPath + "large.txt", 6)
  print("Score so far: " + str(score))
  
  score += 4*testInput(dirPath + "verylarge.txt", 1)
  print("Correctness score: " + str(score) + " out of 30.")

  # Test very large graphs
  print("\n=======================  TEST EFFICIENCY  =======================\n")

  score += 2*testInput(dirPath + "huge1.txt", 1)
  print("Score so far: " + str(score) + " out of 40.")

  score += 2*testInput(dirPath + "huge2.txt", 1)
  print("Score so far: " + str(score) + " out of 40.")

  score += 2*testInput(dirPath + "huge3.txt", 1)
  print("Score so far: " + str(score) + " out of 40.")

  score += 2*testInput(dirPath + "huge4.txt", 1)
  print("Score so far: " + str(score) + " out of 40.")

  score += 2*testInput(dirPath + "huge5.txt", 1)
