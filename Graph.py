...
class Graph:
    ...
    def findEuler(self):
        ...
        """
        if self.isConnected() and not self.anyOdd():
            eulerWalk = Walk(self.totalE+1)
            if self.tryVisiting(0, self.totalEdges(), eulerWalk):
                return eulerWalk
        return None
    ...
    def tryVisiting(self, vertex, unvisitedEs, walk):
        ...
        """
        localUnv = unvisitedEs
        walk.addVertex(vertex)
        if not walk.totalVertices() == 1 and walk.getVertex(0) == walk.getVertex(walk.totalVertices() - 1) and localUnv == 0:
            return True
        for i in range(self.totalV):
            if self.unvisitedE[vertex][i] > 0:
                if self.directed == Graph.UNDIRECTED and not i == vertex:
                    self.unvisitedE[i][vertex] -= 1
                self.unvisitedE[vertex][i] -= 1
                unvisitedEs = localUnv - 1
                if self.tryVisiting(i, unvisitedEs, walk):
                    return True
            if i == self.totalV-1:
                rebound = walk.totalVertices() - 1
                if self.directed == Graph.UNDIRECTED and not walk.getVertex(rebound) == walk.getVertex(rebound-1):
                    self.unvisitedE[walk.getVertex(rebound)][walk.getVertex(rebound-1)] += 1
                self.unvisitedE[walk.getVertex(rebound-1)][walk.getVertex(rebound)] += 1
